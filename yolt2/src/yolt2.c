...
void print_yolt_detections2(FILE **fps, char *id, box *boxes, float **probs, int total, int classes, int w, int h, float min_retain_prob)
{
    // THIS FUNCTION WITH FAIL IF CLASS NAMES INCLUDE '/' CHARACTER!
    //fprintf(stderr, "print_yolt_detections2() test0: total = %d \n", total);

    int i, j;
    for (i = 0; i < total; ++i)
    {
        //fprintf(stderr, " print_yolt_detections2() test1: i = %d \n", i);
        ...
        for (j = 0; j < classes; ++j)
        {
            //fprintf(stderr, "  print_yolt_detections2() test2.0: total = %d, i = %d, j = %d \n", total, i, j);
            //fprintf(stderr, "  print_yolt_detections2() test2.1: probs[i][j] = %f \n", probs[i][j]);
            //fprintf(stderr, "  print_yolt_detections2() test2.2: xmin, ymin, xmax, ymax = %f %f %f %f \n", xmin, ymin, xmax, ymax);
            // Sometimes an error ocuurs in the line below that gives a seg fault
            if (probs[i][j])
            {
                //fprintf(stderr, "  print_yolt_detections2() test2.0: total = %d, i = %d, j = %d \n", total, i, j);
                //fprintf(stderr, "  print_yolt_detections2() test3: probs[i][j] = %f \n", probs[i][j]);
                //fprintf(stderr, "  fps[j] %s \n", fps[j]);
                if (probs[i][j] > min_retain_prob)
                {
                    fprintf(fps[j], "%s %f %f %f %f %f\n", id, probs[i][j],
                            xmin, ymin, xmax, ymax);
                }
            }
            //fprintf(stderr, "  print_yolt_detections2() succeeded \n");
        }
    }
}

void validate_yolt2(char *cfgfile, char *weightfile, char *valid_list_loc,
                    float iou_thresh, char *names[], char *results_dir, int nbands, float min_retain_prob)
{
    int debug = 0;
    int *map = 0;
    network net = parse_network_cfg(cfgfile);
    ...
    for (i = nthreads; i < m + nthreads; i += nthreads)
    {
        ...
        for (t = 0; t < nthreads && i + t - nthreads < m; ++t)
        {
            ...
            print_yolt_detections2(fps, id, boxes, probs, l.w * l.h * l.n, classes, w, h, min_retain_prob);
...
void run_yolt2(int argc, char **argv)
{
    ...
    if (0 == strcmp(argv[2], "test"))
    {
        ...
        train_yolt2(cfg, weights, train_images, results_dir, nbands, loss_file);
    else if (0 == strcmp(argv[2], "valid"))
        validate_yolt2(cfg, weights, valid_list_loc,
