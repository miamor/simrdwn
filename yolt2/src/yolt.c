...
// TRAIN DETECTOR.C.  THEREFORE THIS FILE IS A MODIFIED VERSION OF DETECTOR.C
...
void train_yolt(char *cfgfile, char *weightfile, char *train_images, char *results_dir, int nbands, char *loss_file) //, int *gpus, int ngpus)//, int reset_seen)
{
    int reset_seen = 1;
    int ngpus = 1;
    int *gpus;
    ...
    if (lossfile == NULL)
    {
        fprintf(stderr, "I couldn't open lossfile for appending.\n");
        exit(0);
    }
...
void print_yolt_detections(FILE **fps, char *id, box *boxes, float **probs, int total, int classes, int w, int h)
{
    int i, j;
    for (i = 0; i < total; ++i)
    {
...
void validate_yolt(char *cfgfile, char *weightfile, char *valid_image, char *valid_list_loc,
                   float iou_thresh, char *names[], char *results_dir, int nbands)
{
    ...
    fprintf(stderr, "Learning Rate: %g, Momentum: %g, Decay: %g\n", net.learning_rate, net.momentum, net.decay);
    fprintf(stderr, "valid_image: %s, valid_list_loc: %s\n", valid_image, valid_list_loc);
    ...
    {
        char buff[1024];
        fprintf(stderr, "Output file: %s/%s_%s.txt\n", results_dir, valid_image, names[j]);
        snprintf(buff, 1024, "%s/%s_%s.txt", results_dir, valid_image, names[j]);
        fps[j] = fopen(buff, "w");
    }
    ...
    for (i = nthreads; i < m + nthreads; i += nthreads)
    {
        ...
        for (t = 0; t < nthreads && i + t - nthreads < m; ++t)
        {
            ...
            if (nms)
                do_nms_sort(boxes, probs, l.w * l.h * l.n, classes, iou_thresh);
            ...
            print_yolt_detections(fps, id, boxes, probs, l.w * l.h * l.n, classes, w, h);
...
void test_yolt(char *cfgfile, char *weightfile, char *filename, float plot_thresh, float iou_thresh,
               char *names[], image *voc_labels, int CLASSNUM, int nbands, char *results_dir)
{
    ...
    while (1)
    {
        ...
        image im3 = rgb_from_multiband(im); //copy_image(im);

        // // if using multiband...
        // 		// get 3 band image
        // 		if (nbands > 3){
        // 			image im3 = rgb_from_multiband(im);
        // 		}
...
void run_yolt(int argc, char **argv)
{
    ...
    char *valid_list_loc = (argc > 11) ? argv[11] : 0;
    char *names_str = (argc > 12) ? argv[12] : 0;
    int len_names = (argc > 13) ? atoi(argv[13]) : 0;
    int nbands = (argc > 14) ? atoi(argv[14]) : 0;
    char *loss_file = (argc > 15) ? argv[15] : 0;
    ...
    if (0 == strcmp(argv[2], "test"))
    {
        ...
        test_yolt(cfg, weights, test_filename, plot_thresh, nms_thresh, names, voc_labels, len_names,
                  nbands, results_dir);
    }
    else if (0 == strcmp(argv[2], "train"))
        train_yolt(cfg, weights, train_images, results_dir, nbands, loss_file);
    else if (0 == strcmp(argv[2], "valid"))
        validate_yolt(cfg, weights, valid_image, valid_list_loc,
